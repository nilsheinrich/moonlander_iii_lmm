...
my_data = DataFrame(Arrow.Table("data/Experiment3_DistantFixations.arrow"))
...
```
m_varyingSlope_complex_zc wins then referring to BIC (not AIC though).
...
```
m_varyingSlope_complex_zc is still favoured. We will refer to m_varyingSlope_complex_zc for hypothesis testing.
...
plt = data(tbl) * mapping(
  [:β2, :β3] .=> "Bootstrap replicates of main effect estimates";
  color=dims(1) => renamer([ "N_obstacles", "input_noise"])
  ) * AoG.density()
...
**N_visible_obstacles** increases fixation duration in distant fixations (0.0199164, 0.0527637).
**input noise (numeric)** has no significant effect on fixation duration in distant fixations.
...
m_varyingInt1 = let
    formula = @formula(1/distance_to_spaceship ~ 1 + N_visible_obstacles + input_noise + (1 | ID));
    fit(MixedModel, formula, my_data; contrasts=my_cake);
  end
...
m_varyingSlope_complex = let
    formula = @formula(1/distance_to_spaceship ~ 1 + N_visible_obstacles + input_noise + (1 + N_visible_obstacles + input_noise | ID));
    fit(MixedModel, formula, my_data; contrasts=my_cake);
  end
...
```
We will compare this model with the next one.
...
```
Close but, zero correlation is favored by BIC (and by AIC). Therefore. proceeding with m_varyingSlope_complex_zc.
...
```
Close, but BIC (and AIC) favors keeping input noise as random effect.
...
```
N_visible_drift_tiles can also be kept as random effect. We will refer to m_varyingSlope_complex_zc for hypothesis testing.
...
We find **no** significance for input noise. We however find that **N_visible_obstacles** is significantly **increaing** distance to the spaceship within distant fixations (0.000833239, 0.00210256).
...
issingular(m_varyingSlope_complex) # good to go
...
issingular(m_varyingSlope_complex_zc) # Also good
...
gof_summary = let
  nms = [:m_varyingSlope_complex, :m_varyingSlope_complex_zc, :m_varyingInt1]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope_complex, m_varyingSlope_complex_zc, m_varyingInt1)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
...
```
m_varyingSlope_complex_zc is favored by BIC (not by AIC though, here _complex wins).

Referring to m_varyingSlope_complex_zc for hypothesis testing.
...
**input_noise** has no significant effect on the distance to the closest obstacle in distant fixations (-0.0227808, 0.0474431).
